...
import "./UnblockCardApproved.scss";

const UnblockCardApproved = () => {
  ...
  const approvalReport = useSelector(
    (state: RootStateOrAny) =>
      state.UnblockCardApprovedReducer?.getUnblockResponseReport
  );
  const reportsData = approvalReport?.data;

  const fetchApprovalResponse = useCallback(
    (value: any) => {
      try {
  ...
  useEffect(() => {
    if (approvalReport) {
      if (approvalReport?.data) {
        setIsLoading(false);
      }
    }
  }, [approvalReport]);

  const UnBlockCardApprovedDownloadPdf = useSelector(
    (state: RootStateOrAny) =>
      state.UnblockCardApprovedReducer?.downloadPdfResponse
  );
  const UnBlockCardApprovedDownloadExcel = useSelector(
    (state: RootStateOrAny) =>
      state.UnblockCardApprovedReducer?.downloadExcelResponse
  );

  const fetchUnBlockCardApprovedDownloadReport = useCallback(
  ...
  useEffect(() => {
    if (UnBlockCardApprovedDownloadPdf?.data) {
      setIsLoading(false);
      if (UnBlockCardApprovedDownloadPdf?.data?.reportURL) {
        window.location.href = UnBlockCardApprovedDownloadPdf?.data?.reportURL;
      }
      dispatch(resetCardUnBlockDoenloadPdf());
    } else if (UnBlockCardApprovedDownloadPdf?.error) {
      setIsLoading(false);

      dispatch(resetCardUnBlockDoenloadPdf());
    }
  }, [UnBlockCardApprovedDownloadPdf]);

  useEffect(() => {
    if (UnBlockCardApprovedDownloadExcel?.data) {
      setIsLoading(false);
      if (UnBlockCardApprovedDownloadExcel?.data?.reportURL) {
        window.location.href =
          UnBlockCardApprovedDownloadExcel?.data?.reportURL;
      }
      dispatch(resetCardUnBlockDoenloadExcel());
    } else if (UnBlockCardApprovedDownloadExcel?.error) {
      setIsLoading(false);

      dispatch(resetCardUnBlockDoenloadExcel());
    }
  }, [UnBlockCardApprovedDownloadExcel]);
  ...
  const downloadPdf = () => {
    fetchUnBlockCardApprovedDownloadReport(filterValue, filePdf);
  };

  const downloadExcel = () => {
    fetchUnBlockCardApprovedDownloadReport(filterValue, fileExcel);
  };
    ...
    <div className="p-4">
      <>
        <CommonHeaderSummaryReports
          RightContent={"SRF UnBlock Card - Approved"}
        ...
        {filterOption && (
          <div className="colorWhite UnblockCardApproved mt-3 p-3">
            ...
            <div className="container-fluid filterInputAlign">
              <div className="d-flex row">
                <div className="col-3">
                  <label className="UnBlockCardApproved-label">
                    Mobile Number
                  </label>
                  <div className="col-8 row">
                    <Input
                      className="border-1 UnblockCardApproved-inputcode form-select"
                      type="select"
                      name="inputCode"
              ...
              <div className="row mt-2">
                ...
                <div className="col-3">
                  <FormGroup>
                    ...
                    <Input
                      type="select"
                      className="UnblockCardApproved-select-box form-select"
                      onChange={handleChange}
                      name="requestChannel"
...
export default UnblockCardApproved;

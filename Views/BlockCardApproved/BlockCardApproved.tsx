...
import "./BlockCardApproved.scss";

const BlockCardApproved = () => {
  ...
  const cardBlockReportsApproved: any = useSelector(
    (state: RootStateOrAny) =>
      state.CardBlockReportsReducer.getcardBlockReportsResponse
  );
  const reportsData = cardBlockReportsApproved?.data;

  useEffect(() => {
    if (cardBlockReportsApproved) {
      if (cardBlockReportsApproved?.data) {
        setIsLoading(false);
      }
    }
  }, [cardBlockReportsApproved]);
  ...
  const BlockCardApprovedDownloadPdf = useSelector(
    (state: RootStateOrAny) =>
      state.CardBlockReportsReducer?.getcardBlockReportPdfResponse
  );
  const BlockCardApprovedDownloadExcel = useSelector(
    (state: RootStateOrAny) =>
      state.CardBlockReportsReducer?.getcardBlockReportExcelResponse
  );
  const fetchBlockCardApprovedDownloadReport = useCallback(
  ...
  useEffect(() => {
    if (BlockCardApprovedDownloadPdf?.data) {
      setIsLoading(false);
      if (BlockCardApprovedDownloadPdf?.data?.reportURL) {
        window.location.href = BlockCardApprovedDownloadPdf?.data?.reportURL;
      }
      dispatch(resetPDFRecords());
    } else if (BlockCardApprovedDownloadPdf?.error) {
      setIsLoading(false);

      dispatch(resetPDFRecords());
    }
  }, [BlockCardApprovedDownloadPdf]);

  useEffect(() => {
    if (BlockCardApprovedDownloadExcel?.data) {
      setIsLoading(false);
      if (BlockCardApprovedDownloadExcel?.data?.reportURL) {
        window.location.href = BlockCardApprovedDownloadExcel?.data?.reportURL;
      }
      dispatch(resetExcelRecords());
    } else if (BlockCardApprovedDownloadExcel?.error) {
      setIsLoading(false);

      dispatch(resetExcelRecords());
    }
  }, [BlockCardApprovedDownloadExcel]);

  let filePdf = "pdf";
  let fileExcel = "Excel";

  const downloadPdf = () => {
    fetchBlockCardApprovedDownloadReport(filterValue, filePdf);
  };

  const downloadExcel = () => {
    fetchBlockCardApprovedDownloadReport(filterValue, fileExcel);
  };
    ...
    <div className="p-4">
      <>
        <CommonHeaderSummaryReports
          RightContent={"SRF Block Card - Approved"}
        ...
        {filterOption && (
          <div className="colorWhite BlockCardApproved mt-3 p-3">
            ...
            <div className="container-fluid filterInputAlign">
              <div className="d-flex row">
                <div className="col-3">
                  <label className="BlockCardApproved-label">
                    Mobile Number
                  </label>
                  <div className="col-8 row">
                    <Input
                      className="border-1 BlockCardApproved-inputcode form-select"
                      type="select"
                      name="inputCode"
                    ...
                    <Input
                      className="BlockCardApproved-input"
                      type="number"
              ...
              <div className="row mt-2">
                <div className="ps-0 col-3">
                  <FormGroup>
                    ...
                    <Input
                      type="select"
                      className="form-select BlockCardApproved-select-box"
                      onChange={handleChange}
                      name="requestReason"
...
export default BlockCardApproved;

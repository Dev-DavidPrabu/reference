...
import { getCustomerWalletHistoryList } from "../../redux/action/CustomerEnquiryAction";

function CustomerWalletHistory() {
  ...
  const Header = [
    {
      ...
      render: (e: any) => {
        return <label>{e?.accountTypeName+" to "+e?.newAccountTypeName}</label>;
      },
    },
    {
      title: "Updated By",
...

    {
      ...
      render: (updatedDate: any) => {
        let value = updatedDate?.replace("T"," ");
        return (
          <label>
            {value}
          </label>
  ...
  const CustomerWalletHistory: any = useSelector(
    (state: RootStateOrAny) =>
      state.KYCCustomerEnquiryReducer.getCustomerWalletHistoryData
  );

  let customerWalletHistoryData = CustomerWalletHistory?.data;

  const fetchCustomerData = useCallback(async () => {
    try {
      dispatch(
        getCustomerWalletHistoryList(location?.state?.value?.customerId)
      );
    } catch (err) {}
  }, [dispatch]);
  ...
  if (searchUserData && searchCategory) {
    if (searchCategory === "any") {
      customerWalletHistoryData = customerWalletHistoryData.filter((e: any) => {
        return (
  ...
  return (
    <div className="CustomerWalletHistory">
      <div className="p-3 pb-0">
        <CommonHeaderSummary
          RightContent={"Customer Wallet History"}
          SummaryFileName={"Customer Wallet History"}
          SummaryColumn={orginalColumns.length > 0 ? orginalColumns : Header}
          TableData={customerWalletHistoryData}
          Print={handlePrint}
          addButton={false}
      ...
      {searchArea && (
        ...
        >
          ...
          >
            ...
            <option value="updatedDate" className="cursor">
              {"Updated Date & Time"}
            </option>
      ...
      {isLoading ? null : (
        <div className="px-3 mb-5" ref={componentRef}>
          <CustomTable
            ...
            CustomTableHeader={customerWalletHistoryData}
            DisableMange={true}

...
const WalletError = () => {
  ...
  const errorWalletReport = useSelector(
    (state: RootStateOrAny) =>
      state.WalletUpgradeReducer?.getWalletResponseReport
  );
  let reportsData = errorWalletReport?.data;

  const fetchErrorWalletResponse = useCallback(
    (value: any) => {
      try {
  ...
  useEffect(() => {
    if (errorWalletReport) {
      if (errorWalletReport?.data) {
        setTable(true);
        setIsLoading(false);
      }
    }
  ...
  const errorWalletDownloadReport = useSelector(
    (state: RootStateOrAny) =>
      state.WalletUpgradeReducer?.downloadWalletPdfResponse
  );

  const fetchErrorWalletDownloadResponse = useCallback(
  ...
  useEffect(() => {
    if (errorWalletDownloadReport) {
      if (errorWalletDownloadReport?.data) {
        setTable(true);
      }
    }
  }, [errorWalletDownloadReport]);
  ...
  const exportToData = (apiData: any, type: string) => {
    if (apiData.length !== 0 && errorWalletDownloadReport?.length !== 0) {
      let file = new Blob([apiData?.data], {
        type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      });
      FileSaver.saveAs(file, `WalletErroreports.${type}`);
    }
    resetDownloadDataPdf();
  };
  let DownloadType = errorWalletDownloadReport?.headers?.["content-type"];

  useEffect(() => {
    if (
      errorWalletDownloadReport?.length !== 0 &&
      errorWalletDownloadReport !== undefined
    ) {
      if (DownloadType === "application/pdf") {
        exportToData(errorWalletDownloadReport, ".pdf");
      }
      if (DownloadType === "application/vnd.ms-excel") {
        exportToData(errorWalletDownloadReport, ".xlsx");
      }
      setIsLoading(false);
    }
  }, [errorWalletDownloadReport]);
  ...
  const downloadPdf = () => {
    if (reportsData !== undefined) {
      fetchErrorWalletDownloadResponse(status, filePdf, data);
  ...
  const downloadExcel = () => {
    if (reportsData !== undefined) {
      fetchErrorWalletDownloadResponse(status, fileExcel, data);
    ...
    <div className="p-4">
      <>
        <CommonHeaderSummaryReports
          RightContent={"Wallet Upgrade Report- Error"}
        ...
        {filterOption && (
          <div className="colorWhite WalletUpdateError mt-3 p-3">
            ...
            <div className="container-fluid filterInputAlign">
              <div className="d-flex row">
                <div className="col">
                  <label className="walletError-label">Mobile Number</label>
                  <div className="col-8 row">
                    <Input
                      className="border-1 WalletUpdateError-inputcode form-select"
                      type="select"
                      name="inputCode"
                    ...
                    <Input
                      className="WalletUpdateError-input"
                      type="number"

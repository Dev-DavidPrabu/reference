...
import "./KYCUpdateError.scss";

const KYCUpdateError = () => {
  ...
  const errorKYCReport = useSelector(
    (state: RootStateOrAny) => state.KYCupdateReducer?.getKYCResponseReport
  );

  let reportsData = errorKYCReport?.data;

  const fetchErrorKYCResponse = useCallback(
    (value: any) => {
      try {
  ...
  const errorKYCDownloadReport = useSelector(
    (state: RootStateOrAny) => state.KYCupdateReducer?.downloadKYCPdfResponse
  );

  const fetchErrorKYCDownloadResponse = useCallback(
    (filePdf: any, data: any) => {
      try {
  ...
  useEffect(() => {
    if (errorKYCDownloadReport) {
      if (errorKYCDownloadReport?.data) {
        setIsLoading(true);
        setTable(true);
      }
    }
  }, [errorKYCDownloadReport]);
  ...
  const exportToData = (apiData: any, type: string) => {
    if (apiData.length !== 0 && errorKYCDownloadReport?.length !== 0) {
      let file = new Blob([apiData?.data], {
        type: "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
      });
      FileSaver.saveAs(file, `KycUpdateError.${type}`);
    }
    resetDownloadDataPdf();
  };
  let DownloadType = errorKYCDownloadReport?.headers?.["content-type"];
  ...
  useEffect(() => {
    if (
      errorKYCDownloadReport?.length !== 0 &&
      errorKYCDownloadReport !== undefined
    ) {
      if (DownloadType === "application/pdf") {
        exportToData(errorKYCDownloadReport, ".pdf");
      }
      if (DownloadType === "application/vnd.ms-excel") {
        exportToData(errorKYCDownloadReport, ".xlsx");
      }
      setIsLoading(false);
    }
  }, [errorKYCDownloadReport]);
    ...
    <div className="p-4">
      <>
        <CommonHeaderSummaryReports
          RightContent={"Document Update Report- Error"}
        ...
        />
        {filterOption && (
          <div className="colorWhite KYCUpdateError mt-3 p-3">
            ...
            <div className="container-fluid filterInputAlign">
              <div className="d-flex col row">
                <div className="col-3">
                  <label className="kycUpdateError-label">Mobile Number</label>
                  <div className="col-8 row">
                    <Input
                      className="border-1 KYCUpdateError-inputcode form-select"
                      type="select"
                      name="inputCode"
                    ...
                    <Input
                      className="KYCUpdateError-input"
                      type="number"
              ...
              <div className="row mt-2">
                ...
                <div className="col-6 d-flex justify-content-end mt-4 generate-btn-div">
                  <button
